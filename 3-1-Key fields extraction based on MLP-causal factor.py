...
print("there are %d empty record in column reason." % df['reason'].isnull().sum())

df[df.isnull().values==True]
df = df[pd.notnull(df['article'])]
df = df[pd.notnull(df['reason'])]

print("there are %d empty record in column title." % df['title'].isnull().sum())
print("there are %d empty record in column article." % df['article'].isnull().sum())
print("there are %d empty record in column reason'].isnull().sum())
print("total data: %d ." % len(df))
#Count the number of individual reason
d = {'reason':df['reason'].value_counts().index, 'count': df['reason'].value_counts()}
df_reason = pd.DataFrame(data=d).reset_index(drop=True)


#data pre-processing，Convert the reason class to id, which facilitates the training of  classification models
df['reason_id'] = df['reason'].factorize()[0]#The factorize function can map the nominal type data in the Series as a set of numbers, and the same nominal type maps to the same number.
reason_id_df = df[['reason', 'reason_id']].drop_duplicates().sort_values('reason_id').reset_index(drop=True)#Remove duplicate rows
reason_to_id = dict(reason_id_df.values)
id_to_reason = dict(reason_id_df[['reason_id', 'reason']].values)
...
N = 2
for reason, reason_id in sorted(reason_to_id.items()):
...
#load NER result
df2 = pd.read_excel(r'result-NER.xlsx', index_col=[0])
...
def myPredict(sec):
    ...
    pred_reason_id=clf.predict(tfidf_transformer.transform(vect))
    print(pred_reason_id[0])
    return pred_reason_id[0]

for no,article in df2.iloc[:,0:15].iterrows():
    result = myPredict(article['article'])
    df2.loc[no,"causal factor"] = ''.join(result)
    
print('causal factor model:',clf)
df2.to_excel(r'result-factor.xlsx',encoding="utf_8_sig")
