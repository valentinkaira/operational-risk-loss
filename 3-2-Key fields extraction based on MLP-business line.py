...
print("there are %d empty record in column business." % df['business'].isnull().sum())

df[df.isnull().values==True]
df = df[pd.notnull(df['article'])]
df = df[pd.notnull(df['business'])]

print("there are %d empty record in column title." % df['title'].isnull().sum())
print("there are %d empty record in column article." % df['article'].isnull().sum())
print("there are %d empty record in column business." % df['business'].isnull().sum())
print("total data: %d ." % len(df))
#Count the number of individual business line
d = {'business':df['business'].value_counts().index, 'count': df['business'].value_counts()}
df_business = pd.DataFrame(data=d).reset_index(drop=True)


#data pre-processing，Convert the business class to id, which facilitates the training of  classification models
df['business_id'] = df['business'].factorize()[0]#The factorize function can map the nominal type data in the Series as a set of numbers, and the same nominal type maps to the same number.
business_id_df = df[['business', 'business_id']].drop_duplicates().sort_values('business_id').reset_index(drop=True)#Remove duplicate rows
business_to_id = dict(business_id_df.values)
id_to_business = dict(business_id_df[['business_id', 'business']].values)
...
N = 2
for business, business_id in sorted(business_to_id.items()):
    features_chi2 = chi2(features, labels == business_id)
...
#load NER+reason classification result
df2 = pd.read_excel(r'result-factor.xlsx', index_col=[0])
...
def myPredict(sec):
    ...
    pred_business_id=clf.predict(tfidf_transformer.transform(vect))
    print(pred_business_id[0])
    return pred_business_id[0]

for no,article in df2.iloc[:,0:15].iterrows():
    result = myPredict(article['article'])
    df2.loc[no,"business line"] = ''.join(result)
    
print('business line model:',clf)
df2.to_excel(r'result-factor+business.xlsx',encoding="utf_8_sig")
